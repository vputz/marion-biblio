...
def resultFromQuery(w5, q, context):
    """Given a wos_h5 and a query dict, run all the query elements and
    return a result, which is a dictionary of the query keys and the
    result objects.  Should be changed to JSON for storage
...
def countryCollaborationQuery(w5):
    # this is a collaboration query; in other words we are returning
    # two things: a symmetric matrix, where rows and columns
    # correspond to countries and values represent number of papers
    # countries have collaborated on, and a dictionary mapping
    # "cleaned" country names to row/column indices

    # the result is a matrix in the form of a list (to be JSONable),
    # and a key which is a list of the countries as a legend
...
def categoricalSummaryQuery(w5, queryString):
    """Returns a counter, which is a dictionary of 'label':count
    pairings.  Takes a queryString which is evaluated across the wos
    reader to sum up the quantities by bin.  Due to the fact that the
    queryString is eval'ed, this is titanically dangerous and
    shouldn't be exposed to the public, but it does make the function
    extremely flexible
    ...
    # so now we have a counter, but what we need to return is a sorted
    # list of dictionaries, by value, with "label" and "value" fields
